...
X <- data[, -which(names(data) == "Gross_output_kg")]
y <- data$Gross_output_kg

# Perform Lasso Regression for feature selection
lasso_model <- cv.glmnet(x = as.matrix(X), y = y, alpha = 1)

# Get selected features based on Lasso coefficients
selected_features <- rownames(coef(lasso_model, s = "lambda.min"))[-1]

# Update data with selected features and target variable
data <- data[selected_features]
data$Gross_output_kg <- y
...
dt_predictions_test <- predict(dt_model, newdata = test_data[, -which(names(test_data) == "Gross_output_kg")])
...
svm_predictions_test <- predict(svm_model, newdata = test_data[, -which(names(test_data) == "Gross_output_kg")])
...
gbm_predictions_test <- predict(gbm_model, newdata = test_data[, -which(names(test_data) == "Gross_output_kg")], n.trees = 500)
